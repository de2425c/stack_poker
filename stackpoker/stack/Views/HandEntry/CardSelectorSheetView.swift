...
struct CardSelectorSheetView_Previews: PreviewProvider {
    static var previews: some View {
        StatefulPreviewWrapper(
            singleCard: "As", 
            pairCards: ["Kh", "Qd"],
            tripleCards: ["Tc", "9s", nil]
        ) { singleBinding, pairBinding, tripleBinding in
            VStack {
                Text("Single: \(singleBinding.wrappedValue ?? "None")")
                Text("Pair: \(pairBinding.wrappedValue.compactMap{$0}.joined(separator: ", "))")
                Text("Triple: \(tripleBinding.wrappedValue.compactMap{$0}.joined(separator: ", "))")
...
// Helper for previews with @State, adjusted for three bindings
struct StatefulPreviewWrapper<Content: View>: View {
    @State var singleCardState: String?
    @State var pairCardsState: [String?]
    @State var tripleCardsState: [String?]
    
    var content: (_ singleBinding: Binding<String?>, 
                  _ pairBinding: Binding<[String?]>,
                  _ tripleBinding: Binding<[String?]>) -> Content

    init(singleCard: String?, 
         pairCards: [String?],
         tripleCards: [String?], 
         @ViewBuilder content: @escaping (_ singleBinding: Binding<String?>, 
                                         _ pairBinding: Binding<[String?]>,
                                         _ tripleBinding: Binding<[String?]>) -> Content) {
        _singleCardState = State(initialValue: singleCard)
        _pairCardsState = State(initialValue: pairCards)
        _tripleCardsState = State(initialValue: tripleCards)
        self.content = content
    }

    var body: some View {
        content($singleCardState, $pairCardsState, $tripleCardsState)
    }
} 