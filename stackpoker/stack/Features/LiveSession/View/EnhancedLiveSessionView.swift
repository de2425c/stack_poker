...
struct EnhancedLiveSessionView: View {
                        ...
                        VStack(spacing: 24) {
                            ...
                            if !cashGameService.cashGames.isEmpty {
                                VStack(alignment: .leading, spacing: 12) {
                                    Text("Select from Existing Games")
                                        .font(.plusJakarta(.headline, weight: .medium))
                                        .foregroundColor(.white)
                                        .padding(.leading, 6)
                                    
                                    LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
                                        ForEach(cashGameService.cashGames, id: \.id) { game in
                                            Button(action: {
                                                promptGameName = game.name
                                                promptStakes = game.stakes
                                            }) {
                                                VStack(spacing: 6) {
                                                    Text(game.name)
                                                ...
                                                .background(
                                                    RoundedRectangle(cornerRadius: 12)
                                                        .fill(promptGameName == game.name && promptStakes == game.stakes ? 
                                                              Color.blue.opacity(0.3) : Color.white.opacity(0.1))
                                                        .overlay(
                                                            RoundedRectangle(cornerRadius: 12)
                                                                .stroke(promptGameName == game.name && promptStakes == game.stakes ? 
                                                                        Color.blue : Color.white.opacity(0.3), lineWidth: 1)
                                                        )
                                                )
                                            }
                                        }
                            ...
                            // Manual Game Details Fields
                            VStack(alignment: .leading, spacing: 16) {
                                VStack(alignment: .leading, spacing: 8) {
                                    Text(cashGameService.cashGames.isEmpty ? "Game Details" : "Enter New Game Details")
                                        .font(.plusJakarta(.headline, weight: .medium))
                                        .foregroundColor(.white)
                                        .padding(.leading, 6)
                    ...
                    if selectedLogType == .tournament {
                        VStack(alignment: .leading, spacing: 12) { // This is the VStack for tournament content
                            ...
                            GlassyInputField(
                                ...
                                content: AnyGlassyContent(TextFieldContent(text: $tournamentName, keyboardType: .default, textColor: .white)),
                                glassOpacity: 0.01,
                                labelColor: .gray,
                                materialOpacity: 0.2
                            )
    ...
    // Custom Tab Bar for live session navigation
    private var customTabBar: some View {
        LiveSessionTabBar(selectedTab: $selectedTab, isPublicSession: isPublicSession)
    }
    
    // Empty Notes View when no notes exist
    private var emptyNotesView: some View {
        EmptyNotesView(showingSimpleNoteEditor: $showingSimpleNoteEditor)
    }
    ...
    private func saveGameDetailsAndEndSession() {
        ...
        // Close the prompt and proceed with cashout prompt
        showingGameDetailsPrompt = false
        
        // Initialize cashout amount appropriately for the prompt
        // For tournaments, start with empty string to let user enter amount
        // For cash games, use current chip amount as default
        cashoutAmount = isTournamentSession ? "" : String(Int(sessionStore.enhancedLiveSession.currentChipAmount))
        
        // Show the cashout prompt so user can enter their actual cashout amount
        showingCashoutPrompt = true
    }
    ...
    private func createPublicSession() {
        ...
        // Get proper user name - prefer displayName, fallback to username, then "Unknown"
        let userName: String = {
            if let displayName = userService.currentUserProfile?.displayName, !displayName.isEmpty {
                return displayName
            } else if let username = userService.currentUserProfile?.username, !username.isEmpty {
                return username
            } else {
                return "Unknown"
            }
        ...
        let publicSessionData: [String: Any] = [
            ...
            "createdAt": Timestamp(date: sessionStore.liveSession.startTime)
        ]
        ...
        Task {
            do {
                try await Firestore.firestore().collection("public_sessions").document(sessionId).updateData([
                    ...
                    "lastUpdated": Timestamp(date: Date()),
                    "createdAt": Timestamp(date: sessionStore.liveSession.startTime) // Ensure createdAt is set to session start time
                ])
